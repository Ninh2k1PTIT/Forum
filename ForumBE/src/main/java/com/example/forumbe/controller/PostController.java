...
public class PostController {
    ...
    @GetMapping("/post-by-tag/{id}")
    public DataResponse<List<PostDTO>> getByTagId(@PathVariable(value = "id") Long id) {
        return postService.getByTagId(id);
    }
    ...
    public PostDTO updateById(@PathVariable(value = "id") Long id, @RequestBody PostDTO request) {
        ...
        postDTO.setTags(request.getTags());
        postDTO.setUpdatedAt(new Date());
        postDTO.setClosedAt(request.getClosedAt());
