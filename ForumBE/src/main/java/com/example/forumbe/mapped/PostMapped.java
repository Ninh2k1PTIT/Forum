...
public class PostMapped {
    ...
    public static Post convertToEntity(PostDTO postDTO) {
        ...
        post.setPostVotes(postDTO.getPostVotes().stream().map(item -> PostVoteMapped.convertToEntity(item)).collect(Collectors.toList()));
        post.setTags(postDTO.getTags().stream().map(item -> TagMapped.converToEntity(item)).collect(Collectors.toList()));
        return post;
    }
